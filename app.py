...
# FIX: Bounded cache to prevent memory leak
_order_cache = []
MAX_CACHE_SIZE = 100


def build_order_summary(items):
    """Build order summary and cache it for analytics, with eviction to prevent memory leaks."""
    summary = {
        'items': list(items),
...
def checkout():
    try:
        ...
        build_order_summary(cart_items)
